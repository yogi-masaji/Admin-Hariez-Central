...
import ModalUpdate from '../sections/@dashboard/pembayaran/UserModalUpdate';
import ModalPostButton from '../sections/@dashboard/pembayaran/UserModalPost';
import ModalDeleteButton from '../sections/@dashboard/pembayaran/UserModalDelete';
// ----------------------------------------------------------------------
...
const TABLE_HEAD = [
  ...
  { id: 'Metode Pembayaran', label: 'Metode Pembayaran', alignRight: false },
  { id: 'Teknisi', label: 'Teknisi', alignRight: false },
  { id: 'Biaya', label: 'Biaya', alignRight: false },
  { id: 'Tanggal Pembayaran', label: 'Tanggal Pembayaran', alignRight: false },
  { id: 'Status', label: 'Status', alignRight: false },
  { id: 'Bukti Pembayaran', label: 'Bukti Pembayaran', alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  // });
  if (query) {
    return filter(array, (_user) => _user.PerbaikanNew.kodePerbaikan.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
  return array;
}
...
export default function PerbaikanPage() {
  ...
  useEffect(() => {
    const fetchPelanggan = async () => {
      try {
        ...
        const response = await fetch('http://127.0.0.1:3000/pembayaran', { headers });
        if (!response.ok) {
          throw new Error('Failed to fetch user data');
        }

        const data = await response.json();
        const perbaikanData = data['Data Pembayaran'];
  ...
  const DeletePelanggan = (id) => {
    const token = Cookies.get('token');
    axios
      .delete(`http://127.0.0.1:3000/pembayaran/${id}`, { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
        console.log(res);
  ...
  const UpdatePelanggan = (id, updatedData) => {
    const token = Cookies.get('token');
    axios
      .put(`http://127.0.0.1:3000/pembayaran/konfirmasi/${id}`, updatedData, {
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1}>
                        ...
                        <TableCell onClick={() => handleImageClick(image)}>
                          <img src={image} alt={image} style={{ width: '50px', height: 'auto', cursor: 'pointer' }} />
                        </TableCell>
                        <ImageModal
