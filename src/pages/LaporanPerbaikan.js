...
const TABLE_HEAD = [
  ...
  { id: 'Bulan', label: 'Bulan', alignRight: false },
  { id: 'Jumlah Perbaikan', label: 'Jumlah Perbaikan', alignRight: false },
  ...
  useEffect(() => {
    const fetchPelanggan = async () => {
      try {
        ...
        const response = await fetch('http://127.0.0.1:3000/perbaikan/laporan', { headers });
        if (!response.ok) {
          throw new Error('Failed to fetch user data');
        }

        const data = await response.json();
        const perbaikanData = data['Data Perbulan'];
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => {
                    const { key, bulan, tahun, TotalBiaya, TotalPerbaikan, komplain } = row;
                    ...
                    return (
                      <TableRow hover key={key} tabIndex={-1}>
                        ...
                        <TableCell align="left">{row.bulan}</TableCell>
                        <TableCell align="left">{row.totalPerbaikan}</TableCell>
