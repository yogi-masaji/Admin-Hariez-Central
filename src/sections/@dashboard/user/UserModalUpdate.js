...
const ModalUpdate = ({ onUpdateModal, id }) => {
  ...
  const handleOpen = async () => {
    try {
      ...
      const response = await axios.get(`http://127.0.0.1:3000/user/${id}`, { headers });
      const dataPelanggan = response.data.dataPelanggan;

      setNama(dataPelanggan.nama);
      setNomorTelepon(dataPelanggan.nomorTelepon);
  ...
  const handleUpdateDataPelanggan = () => {
    setLoading(true);
    const token = Cookies.get('token');
    const headers = {
      Authorization: `Bearer ${token}`,
    ...
    axios
      .put(`http://127.0.0.1:3000/user/update/admin/${id}`, updatePelanggan, { headers })
      .then((response) => {
    ...
    <>
      ...
      >
        <Box sx={style}>
          ...
          />
          <Button
            onClick={handleUpdateDataPelanggan}
            variant="contained"
